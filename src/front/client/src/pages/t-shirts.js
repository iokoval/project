    ...
    useEffect(() => {
        async function load() {
            await api.getAllProducts().then(products => {
                setProducts(products.data.filter(product => (product.type === 't-shirt')))
            })
        }
        load();
    }, [])
    ...
    return (
        <Grid container className="contentWrapper" spacing={1} style={{'width': '70%', margin: 'auto'}}>
            {products.map((product, i) => (
                <Grid item justify="center" key={i} alignItems="center" xs={2.5} className='contentBlock' style={{margin: '10px 10px'}}>
                    <Paper className="paper">
                        ...
                        <li className="productBuy"><Button variant="contained" tag={Link} to="/order">Купить</Button></li>
                    </Paper>
                </Grid>
            ))}
        </Grid>
